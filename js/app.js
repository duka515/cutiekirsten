(function () {
  ...
  if (headline && popup.headline) headline.textContent = popup.headline;
  ...
  function openOffer(url, kind) {
    currentUrl = url || popup.url || "";
    if (headline) headline.textContent = headlines[kind] || popup.headline || "Create a free account to continue";
    var kicker = document.getElementById("popupKicker");
    if (kicker) kicker.textContent = kickers[kind] || "Exclusive access";
    if (!overlay) return;
    overlay.hidden = false;
    overlay.style.cssText = "position:fixed;inset:0;z-index:2147483647;display:flex;align-items:center;justify-content:center;background:rgba(8,6,15,.72)";
    document.documentElement.classList.add("funnel-open");
    document.body.classList.add("funnel-open");
  }
  function bind(selector, url, kind) {
  ...
  bind(".js-live", c.liveUrl, "live");
  bind(".js-premium", c.premiumUrl, "premium");
  bind(".js-photos", c.photosUrl, "photos");
  if (join) {
